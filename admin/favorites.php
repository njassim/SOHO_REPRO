...
//$comp_id    =   COMPID($fav_usr_id);
$check_fav  =   CHECKFAV($fav_cmp_id, $fav_prod_id);
if(count($check_fav) > 0){
$query = "DELETE FROM sohorepro_favorites WHERE comp_id = '".$fav_cmp_id."' AND product_id = '".$fav_prod_id."' ";
}  else {
    
    $sql_order_id = mysql_query("SELECT sort_id FROM sohorepro_favorites WHERE comp_id = '".$fav_cmp_id."' ORDER BY sort_id DESC LIMIT 1");
...
if($res_fav){
$check_fav_af  =   CHECKFAV($fav_cmp_id, $fav_prod_id);
