...
if(isset($_POST['off_day_insert']) && $_POST['off_day_insert'] != '')
{ 
    $off_day_insert       = $_POST['off_day_insert'];
    $check_exist          = CheckExistDayOff($off_day_insert);
    if(count($check_exist) < 1){
    $sql = "INSERT INTO sohorepro_day_off
			SET     date = '" . $off_day_insert . "',
                                status = '1' ";
    $result = mysql_query($sql);
    }
    if($result){
        echo 'Off day set successfully~';
    ?>
    <?php 
